...
heat = st.number_input("Enter heat value", value=136, step=1)
...
def fare_cogen(kWh, effi_boil):
    """Calculates the cogeneration fare."""
    cogen_rate = 19.65 * 1.1
    boil_rate = 21.8144 * 1.1
    MJ = 42.7
    out = round((cogen_rate * MJ - MJ * effi_boil * boil_rate) / gen_kW * 1.1, 0)
    return out
...
def analyze_costs(month, gen_kW, household_count, effi_boil, total_kWh, heat):
    ...
    for i in range(total_kWh, 0, -1):
        ...
        if heat > heat_cogen:
            effi_boil_re = effi_boil
        else:
            effi_boil_re = effi_boil * heat / heat_cogen

        cost_cogen = fare_cogen(1, effi_boil_re) * (total_kWh - i)
        cost_elec = calculate_electricity_bill(i, month)
        total = cost_cogen + cost_elec
        data.append([month, i, cost_elec, cost_cogen, total, heat, heat_cogen])

    df_table = pd.DataFrame(data, columns=['month', 'i', 'cost_elec', 'cost_cogen', 'total', 'heat', 'heat_cogen'])
    df_table = df_table.astype(int)

    saving = round(df_table['total'][0] - df_table['total'].min(), 0)
    columns_to_multiply = ['i', 'cost_elec', 'cost_cogen', 'total', 'heat', 'heat_cogen']
    df_table2 = df_table[columns_to_multiply].apply(lambda x: x * household_count/1000).astype(int)
...
def main():
    ...
    if st.button("Run Analysis"):
                ...
                chart = px.line(df_table2, x='i', y=['cost_elec', 'cost_cogen', 'total'])
                ...
                chart.update_xaxes(title_text='MWh')
